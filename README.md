...
The schemas in this project are derived automatically from the source JSON data
using pandas.json_normalize during JSON → Parquet conversion.

Because the source data contains nested objects and arrays, the resulting schema
is a flattened analytics-oriented schema rather than a manually normalized
relational model.

The final schemas were generated by inspecting the Parquet outputs and are
documented explicitly in the schemas/ directory. Nested high-cardinality arrays
are retained as JSON where full normalization is not required for current
